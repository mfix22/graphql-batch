/**
 * NOTE this was forked from https://github.com/calebmer/graphql-resolve-batch/blob/master/src/batch.js
 * PR open here: https://github.com/calebmer/graphql-resolve-batch/pull/7
 */
/**
 ...
 */
function createBatchResolver(batchResolveFn, compositeKeyFn = defaultCompositeKey) {
  ...
  const batcher = new Batcher(batchResolveFn, compositeKeyFn)
  return (source, args, context, info) => {
    return new Promise((resolve, reject) => {
      batcher.batch(source, args, context, info, resolve, reject, compositeKeyFn)
    })
  }
}
 ...
 */
class Batcher {
  constructor(batchResolveFn, compositeKeyFn) {
    this._batchResolveFn = batchResolveFn
    this._batches = new Map()
    this._compositeKeyFn = compositeKeyFn
  }
  ...
  batch(source, args, context, info, resolve, reject) {
    ...
    // the other fields.
    const key = this._compositeKeyFn(source, args, context, info)
  ...
  _resolve() {
    ...
    this._batches.forEach(batch => {
      ...
      executePromise(this._batchResolveFn, batch.sources, batch.args, batch.context, batch.info)
        .then(
...
module.exports = createBatchResolver
exports = createBatchResolver
exports.default = createBatchResolver

exports.defaultCompositeKey = defaultCompositeKey
